...
const ItemModal = ({
  ...
  const [selectedVariantId, setSelectedVariantId] = useState<string>(
    item.variants.find(v => v.isActive !== false)?.id || item.variants[0]?.id
  );
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-4 animate-in fade-in duration-300" onClick={onClose}>
      <div
        className="bg-gray-50 w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl flex flex-col shadow-2xl overflow-hidden animate-in slide-in-from-bottom duration-300"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header Image */}
        <div className="relative h-24 sm:h-32 shrink-0 bg-white">
          {item.imageURL ? (
          ...
          <div className="absolute bottom-4 left-4 right-4 text-white z-10 flex items-end justify-between">
            <div className="flex-1 pr-4">
              <div className="mb-1"><VegBadge isVeg={item.isVeg} size="sm" /></div>
              <h3 className="text-xl sm:text-2xl font-black tracking-tight leading-tight drop-shadow-md">{item.name}</h3>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 p-3 sm:p-4">
          {item.description && <p className="text-gray-600 text-xs mb-3 leading-relaxed bg-white p-2.5 rounded-xl border border-gray-100 shadow-sm">{item.description}</p>}

          {/* Variant Selection */}
          <div className="mb-4">
            <h4 className="font-black text-gray-900 mb-2 tracking-tight flex items-center gap-2 text-sm">
              Choose Option <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full uppercase tracking-wider">Required</span>
            </h4>
            <div className="space-y-2">
              {item.variants.filter(v => v.isActive !== false).map(variant => (
                <div
                  key={variant.id}
                  onClick={() => setSelectedVariantId(variant.id)}
                  className={`flex items-center justify-between p-2.5 sm:p-3 rounded-xl border-2 cursor-pointer transition-all duration-200 ${selectedVariantId === variant.id ? 'border-blue-600 bg-blue-50/50 shadow-sm shadow-blue-100' : 'border-transparent bg-white shadow-sm hover:border-gray-200'}`}
                >
                  <div className="flex-1">
                    <p className={`font-bold text-sm ${selectedVariantId === variant.id ? 'text-blue-700' : 'text-gray-900'}`}>{variant.variantName}</p>
                    {variant.portionSize && <p className="text-[10px] font-medium text-gray-500 mt-0.5">{variant.portionSize}</p>}
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-sm font-black text-gray-900">{formatPrice(variant.price)}</span>
                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${selectedVariantId === variant.id ? 'border-blue-600 bg-blue-600' : 'border-gray-300'}`}>
                      {selectedVariantId === variant.id && <div className="w-2 h-2 bg-white rounded-full" />}
                    </div>
                  </div>
                </div>
              ))}
          ...
          {availableExtras.length > 0 && (
            <div className="mb-2">
              <h4 className="font-black text-gray-900 mb-2 tracking-tight flex items-center gap-2 text-sm">
                Add-ons <span className="text-[10px] font-bold text-gray-500 bg-gray-200 px-2 py-0.5 rounded-full uppercase tracking-wider">Optional</span>
              </h4>
              <div className="space-y-2">
                {availableExtras.map(extra => {
                  ...
                  return (
                    <div
                      key={extra.id}
                      className={`flex items-center justify-between p-2.5 sm:p-3 rounded-xl border-2 transition-all duration-200 ${selectedExtra ? 'border-blue-600 bg-blue-50/50 shadow-sm shadow-blue-100' : 'border-transparent bg-white shadow-sm hover:border-gray-200'}`}
                    >
                      <div className="flex-1 pr-3">
                        <p className={`font-bold text-xs ${selectedExtra ? 'text-blue-700' : 'text-gray-900'}`}>{extra.name}</p>
                        {extra.description && <p className="text-[10px] font-medium text-gray-500 mt-0.5 leading-relaxed">{extra.description}</p>}
                        <p className="text-xs font-black text-gray-900 mt-1">{formatPrice(extra.price)}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        {selectedExtra ? (
                          ...
                          <button
                            ...
                            className="px-4 py-1.5 text-xs bg-white border border-gray-200 text-blue-600 rounded-lg font-bold shadow-sm hover:bg-blue-50 hover:border-blue-200 transition-colors"
                          >
                            Add
                          </button>
                        )}
        ...
        {/* Footer */}
        <div className="p-3 sm:p-4 bg-white border-t border-gray-100 shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)] z-10">
          <div className="flex items-center justify-between mb-3 px-1">
            <div className="flex items-center bg-gray-100 border border-gray-200 rounded-lg h-9 p-1">
              <button
                onClick={() => setQuantity(Math.max(1, quantity - 1))}
                className="w-8 h-full flex items-center justify-center text-gray-600 hover:bg-white hover:shadow-sm rounded-md transition-all"
              >
                <Minus size={16} strokeWidth={2.5} />
              </button>
              <span className="w-8 text-center font-black text-gray-900 text-base">{quantity}</span>
              <button
            ...
            <div className="text-right flex flex-col items-end">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Item Total</p>
              <p className="text-xl font-black text-gray-900 leading-none">{formatPrice(finalTotal)}</p>
            </div>
          </div>
          <button
            onClick={handleAdd}
            disabled={!selectedVariantId}
            className="w-full bg-gradient-to-r from-blue-600 to-blue-700 text-white font-black py-3 rounded-xl disabled:opacity-50 hover:from-blue-700 hover:to-blue-800 transition-all shadow-md active:scale-[0.98] flex items-center justify-center gap-2 text-base"
          >
            Add item to cart <ArrowRight size={18} />
          </button>
        </div>
      </div>
    </div>
                    ...
                    {cat.items.map(item => {
                      return (
                        <div
                          ...
                          className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden flex flex-col h-full hover:shadow-md transition-shadow cursor-pointer group relative"
                        >
                          <div className="flex justify-between items-start gap-3 p-3 pb-0">
                            <div className="flex-1 min-w-0">
                              <div className={`w-3 h-3 rounded-sm border ${item.isVeg ? 'border-green-600' : 'border-red-600'} flex items-center justify-center mb-1.5`}>
                                <div className={`w-1.5 h-1.5 rounded-full ${item.isVeg ? 'bg-green-600' : 'bg-red-600'}`} />
                              </div>
                              <h3 className="font-bold text-gray-800 text-sm leading-snug mb-0.5 truncate">{item.name}</h3>
                            </div>
                          </div>
                            ...
                            <div className="space-y-1 mb-2">
                              {item.variants.filter(v => v.isActive !== false).map((variant) => (
                                <div key={variant.id} className="flex justify-between items-center text-[11px]">
                                  <span className="text-gray-600 truncate mr-2" title={variant.variantName}>{variant.variantName}</span>
                                  <span className="font-bold text-gray-900 shrink-0">{formatPrice(variant.price)}</span>
                                </div>
                              ))}
                            </div>

                            <div className="mt-2 pt-2 border-t border-dashed border-gray-100 flex items-center justify-between">
                              {item.variants.filter(v => v.isActive !== false).length > 1 ? (
                                <span className="text-[9px] font-bold tracking-wider uppercase bg-gray-50 text-gray-500 px-2 py-0.5 rounded">Options</span>
                              ) : (
                                <div /> // Spacer
                              )}

                              <button className="px-5 py-1.5 bg-white border border-blue-200 text-blue-600 font-extrabold text-xs rounded hover:bg-blue-50 transition-colors shadow-sm">
                                ADD
                              </button>
                            </div>
                          </div>
...
function CartContent({
  cartItems, independentExtras, total, onUpdateQty, onRemove, onUpdateIndepQty, onPlaceOrder, notes, setNotes, clear
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
}: any) {
