...
router.post("/make-order", optionalAuth, async (req: any, res) => {
  try {
    ...
    const user = req.user;
    const { tableSessionId, restaurantId: providedRestaurantId, notes, items, status, discount: frontendDiscount, discountType, discountValue } = req.body;
    ...
    for (const item of regularItems) {
      const variant = variants.find(v => v.id === (item.variantId || item.menuItemVariantId));
      if (!variant) {
        throw new Error(`Menu item variant ${item.variantId || item.menuItemVariantId} not found`);
      }
      ...
      if (item.extras && Array.isArray(item.extras)) {
        for (const extra of item.extras) {
          // Fetch extra details to get current price
          ...
          // Standard F&B logic: Extras apply to EACH item in the group (e.g., 2 Burgers = 2 Extra Cheeses)
    ...
    const gstPercentage = Number(restaurantData[0].defaultGstPercentage) || 0;
    ...
    for (const item of regularItems) {
      ...
      if (item.extras && Array.isArray(item.extras)) {
        for (const extra of item.extras) {
          ...
          const extraPrice = extraDetails[0].price;
          const extraQuantity = (extra.quantity || 1) * item.quantity; // Multiply by parent quantity
    ...
    if (independentExtras.length > 0) {
      ...
      for (const extraItem of independentExtras) {
        ...
        for (const extra of extraItem.extras) {
          ...
          extrasList.push(`${extraQuantity}x ${extraDetails[0].name}`);
        }
      }
