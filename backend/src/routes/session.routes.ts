...
router.post("/create-session", async (req: any, res) => {
  ...
  try {
    ...
    let { tableId, restaurantId, qrToken, latitude, longitude } = req.body;
    ...
    if (qrToken) {
      ...
      // 🗺️ Proximity Geofencing Check
      const restRecord = await db
        .select({
          enforceProximity: restaurants.enforceProximity,
          latitude: restaurants.latitude,
          longitude: restaurants.longitude,
          geofenceRadius: restaurants.geofenceRadius,
      ...
      if (restRecord.length > 0) {
        const rInfo = restRecord[0];
        if (rInfo.enforceProximity && rInfo.latitude && rInfo.longitude) {
          // Destructure coords from req.body or query as fallback
          let custLat = latitude || req.query.latitude;
          let custLon = longitude || req.query.longitude;

          if (!custLat || !custLon) {
            console.warn("⚠️ [CREATE SESSION] Location verification required but coordinates missing");
            return res.status(400).json({
              success: false,
              code: "LOCATION_REQUIRED",
              message: `Location permission is required to verify you are physically at ${rInfo.restaurantName}. Please enable location services.`
            });
          }

          const lat1 = parseFloat(custLat as string);
          const lon1 = parseFloat(custLon as string);
          const lat2 = parseFloat(rInfo.latitude);
          const lon2 = parseFloat(rInfo.longitude);
          ...
          const allowedRadius = rInfo.geofenceRadius || 100;
          console.log(`📍 [PROXIMITY CHECK] Customer is ${distance.toFixed(1)}m away. Allowed: ${allowedRadius}m`);

          if (distance > allowedRadius) {
            return res.status(400).json({
              success: false,
              code: "PROXIMITY_FAILED",
              message: `Proximity validation failed. You are too far (${distance.toFixed(0)}m) from ${rInfo.restaurantName} to order. You must be physically present at the restaurant.`
            });
          }
        }
      }
